...
function Button({ type, className, to, disabled, children }) {
  const base = clsx(
    "inline-block rounded-xl font-semibold uppercase tracking-wide transition-all duration-300 focus:outline-none focus:ring focus:ring-offset-2 disabled:cursor-not-allowed",
  );

  const primary = clsx(
    "bg-yellow-400 px-4 py-3 text-stone-800 hover:bg-yellow-300 focus:ring-yellow-300 sm:px-5 sm:py-4",
  );
  ...
  const small = clsx(
    "bg-yellow-400 px-3 py-2 text-sm text-stone-800 hover:bg-yellow-300 focus:ring-yellow-300 sm:px-4 sm:py-3",
  );
  const link = clsx(
